...
class NVisualization():

    def __init__(self):
        self.Ndata = np.loadtxt(open('NETIMISModel.csv', 'rb'), delimiter=',', skiprows=0, dtype=str)
        # print (self.Ndata)
        self.Ndata_Gender = self.Ndata[:, 1]
        # print (self.Ndata_Gender)
        self.Ndata_Age = self.Ndata[:, 2]
        self.Ndata_Condition = self.Ndata[:, 3]
        self.Ndata_Outcome = self.Ndata[:, 4]
        self.Ndata_Time = self.Ndata[:, 5]
        self.Ndata_Cost = self.Ndata[:, 6]

    def Nbasicinformation(self):
        self.bi = plt.figure()

        self.bi_genderpie = self.bi.add_subplot(1, 3, 1)  # pie chart gender

        self.genderclass = Counter(self.Ndata_Gender)
        ...
        self.ageclass = Counter(self.Ndata_Age)
        ...
        self.conditionclass = Counter(self.Ndata_Condition)
        ...
        # for i in self.conditionclass:
        #     self.numlist_condition.append(self.conditionclass[str(i)])
        #     self.namelist_condition.append(i)
        # print (self.numlist_condition)
        ...
        plt.suptitle('NVisualization Basic Information of Patients (2500 Records from NETIMIS csv files)')
        plt.show()

    def Noutcomedistribution(self):
        self.od = plt.figure()

        self.outcomeclass = Counter(self.Ndata_Outcome)
        # self.numlist_outcome = []
        # self.namelist_outcome = []
        ...
        # for i in self.outcomeclass:
        #     self.numlist_outcome.append(self.outcomeclass[str(i)])
        #     self.namelist_outcome.append(i)
        # print(self.numlist_outcome)
        print(self.namelist_outcome)
        ...
        plt.suptitle('NVisualization Outcome Distribution (2500 Records from NETIMIS csv files)')
        plt.show()

    def Ncostrelation(self):
        self.cr = plt.figure()
        ...
        for i in range(2500):
            for j in self.namelist_outcome:
                if self.Ndata_Outcome[i] == j:
                    self.one_cost = float(self.Ndata_Cost[i])
                    # print (self.one_cost)
        ...
        for i in self.namelist_outcome:
            for j in range(2500):
                if self.Ndata_Outcome[j] == i:
                    self.calculation_list.append(float(self.Ndata_Cost[j]))
        ...
        plt.suptitle('NVisualization Average Cost in Different Outcomes (2500 Records from NETIMIS csv files)')
